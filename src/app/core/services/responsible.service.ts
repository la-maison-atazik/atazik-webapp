...
export class ResponsibleService {
  ...
  private listenToCollection() {
    ...
    const responsibleList$: Observable<Responsible[]> = collectionData(ref, {
      idField: "uid",
    }) as Observable<Responsible[]>;
  ...
  public researchPossibleResponsible(partialResponsible: PartialResponsible): Responsible[] {
    return this.currentResponsibleList.filter((responsible) => {
      ...
      if (partialResponsible.firstName) {
        isMatch =
          isMatch &&
          responsible.firstName.toLowerCase().includes(partialResponsible.firstName.toLowerCase());
      }
      if (partialResponsible.lastName) {
        isMatch =
          isMatch &&
          responsible.lastName.toLowerCase().includes(partialResponsible.lastName.toLowerCase());
      }
      if (partialResponsible.email) {
        isMatch =
          isMatch &&
          responsible.email.toLowerCase().includes(partialResponsible.email.toLowerCase());
      }
      if (partialResponsible.phone) {
        isMatch =
          isMatch &&
          responsible.phone.toLowerCase().includes(partialResponsible.phone.toLowerCase());
      }
      if (partialResponsible.address) {
        isMatch =
          isMatch &&
          responsible.address.toLowerCase().includes(partialResponsible.address.toLowerCase());
      }
      if (partialResponsible.city) {
        isMatch =
          isMatch && responsible.city.toLowerCase().includes(partialResponsible.city.toLowerCase());
      }
      if (partialResponsible.postalCode) {
        isMatch =
          isMatch &&
          responsible.postalCode
            .toLowerCase()
            .includes(partialResponsible.postalCode.toLowerCase());
      }
      return isMatch;
    });
  }
