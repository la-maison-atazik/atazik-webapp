...
import {
  mapFirebaseUsersToRows,
  mapUserInvitesToRows,
} from "../../../core/mappers/user-to-row.mapper";
...
export class UserManagementComponent implements OnInit {
    ...
    await this.userService
      ...
      .catch((error) => {
        this.messageService.add({
          ...
          detail:
            "Une erreur est survenue lors du chargement des invitations. Veuillez réessayer plus tard.",
        });
  ...
  protected openDeleteUserDialog(user: FirebaseUserRow) {
    if (user.status === UserStatusEnum.INVITED) {
      ...
      this.confirmationService.confirm({
        message:
          "Êtes-vous sûr de vouloir supprimer cet utilisateur ? Il ne pourra plus accèder à l'application.",
