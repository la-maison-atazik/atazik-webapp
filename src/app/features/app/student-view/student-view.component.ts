...
export class StudentViewComponent implements OnInit {
	...
	private buildForm(): void {
		this.studentForm = this.fb.nonNullable.group({
			...
			lastName: this.fb.control<string | null>(null, { validators: [Validators.required, Validators.maxLength(70)] }),
			firstName: this.fb.control<string | null>(null, { validators: [Validators.required, Validators.maxLength(70)] }),
	...
	private loadStudent(): void {
		try {
			...
			const toDate = (d: any): Date => (d?.toDate ? d.toDate() : d instanceof Date ? d : new Date(d));
	...
	private newActivityGroup(a?: Partial<Activity>): ActivityFormGroup {
		const group = this.fb.group({
			...
			price: this.fb.control<number | null>(a?.price ?? 0, { validators: [Validators.required, Validators.min(0)] }),
	...
	protected async save(): Promise<void> {
		...
		try {
			...
			this.messageService.add({ severity: "error", summary: "Erreur", detail: "Sauvegarde échouée." });
		}
	}
