...
export class NewRegistrationComponent implements OnInit, OnDestroy {
  ...
  set possibleResponsibleList(value) {
    ...
    this._possibleResponsibleList.push({
      uid: undefined,
      displayName:
        value.length === 0 ? "Aucun responsable existant trouvé" : "Aucun de ces responsables",
      email: "",
    });
  ...
  public ngOnInit() {
    ...
    const birthSub = this.registrationForm
      .get("student.birthDate")
    ...
    const studentNameSub = this.registrationForm
    ...
    // Si un responsable est sélectionné, on ne fait plus de recherche
    // Attente de 500ms après la dernière saisie avant de lancer la recherche
    const responsibleFirstNameSub = this.registrationForm
    ...
    const responsibleLastNameSub = this.registrationForm
    ...
    const responsibleEmailSub = this.registrationForm
  ...
  protected async onSubmit(): Promise<void> {
    ...
    if (
      this.possibleResponsibleSelected === undefined ||
      this.possibleResponsibleSelected.uid === undefined
    ) {
      // Aucun responsable sélectionné, on crée un nouveau responsable
      const responsible = responsibleFromFormRegistration(rawValues);
      const responsibleCollectionRef = collection(
        this.firestore,
        FirestoreCollectionsEnum.RESPONSIBLE,
      );
      ...
      try {
        ...
        this.messageService.add({
          ...
          summary: "Erreur",
          detail:
            "Une erreur est survenue lors de l'enregistrement du responsable. Veuillez réessayer.",
        });
        this.loading = false;
        return;
      }
    ...
    addDoc(studentCollectionRef, student)
      ...
      .catch((err) => {
        ...
        this.messageService.add({
          ...
          summary: "Erreur",
          detail:
            "Une erreur est survenue lors de l'enregistrement de l'inscription. Veuillez réessayer.",
        });
        this.loading = false;
      });
  }
  ...
  protected goBack(force = false): void {
    if (this.hasData && !force) {
      this.confirmationService.confirm({
        message:
          "Vous avez des modifications non enregistrées. Voulez-vous vraiment revenir en arrière ?",
        header: "Confirmation",
  ...
  private updateTotals() {
    ...
    this.totalActivities.set(
      activities.reduce((total, activity) => total + (activity.get("price")?.value ?? 0), 0),
    );
    this.discount.set(
      this.nbActivities() >= this.discountThreshold
        ? this.totalActivities() * this.discountRate
        : 0,
    );
  ...
  protected onListResponsibleSelect(event: SelectChangeEvent) {
    ...
    if (selected) {
      ...
      const responsible = this.responsibleService.currentResponsibleList.find(
        (r) => r.uid === selected.uid,
      );
      if (responsible) {
        this.registrationForm.patchValue({
          responsible: {
